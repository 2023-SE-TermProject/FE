...
const ReservationList = ({ date, roomId }) => {
    ...
    useEffect(() => {
        ...
        const reservationTimeUrl = `http://gcu-metaverse.shop:8080/reservations/${roomId}?year=${date.getFullYear()}&month=${date.getMonth()}&date=${date.getDate()}`;
    ...
    function onTimeBtnClick(time) {
        ...
        const timeRangeUrl = `http://gcu-metaverse.shop:8080/reservations/${roomId}/valid-time?year=${date.getFullYear()}&month=${date.getMonth()}&date=${date.getDate()}&start=${time}`
