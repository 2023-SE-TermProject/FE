...
const ReservationList = ({ date, roomId }) => {
    ...
    useEffect(() => {
        ...
        const reservationTimeUrl = `/reservations/${roomId}?year=${date.getFullYear()}&month=${(date.getMonth()+parseInt(1))}&date=${date.getDate()}`;
    ...
    function onTimeBtnClick(time) {
        ...
        const timeRangeUrl = `/reservations/${roomId}/valid-time?year=${date.getFullYear()}&month=${(date.getMonth()+parseInt(1))}&date=${date.getDate()}&start=${time}`
